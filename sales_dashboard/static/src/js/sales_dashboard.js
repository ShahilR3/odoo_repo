...
//            countries: [],
//            selected: '',
//            top_products_table: [],  // NEW table data
//        });
//
//        this.category = useState({
//            categories: [],
...
//
//    async _fetch_data(period = "", country = "", category = "") {
//        const result = await this.orm.call("sale.order", "get_dashboard_data", [period, country, category], {});
//        this.result = result;
//
//        // Chart data
//        this.sales_team = result.sales_team;
...
//        this.topBarGraph();
//
//        this.ls_product = result.ls_product;
//        this.LSPLineGraph();
//
//        this.order_state = result.order_state;
//        this.OSGraph();
//
...
//
//        // NEW Table data
//        this.state.top_products_table = result.top_products.map(p => ({
//            id: p[0],       // unique id
//            name: p[1],     // product name
//            qty: p[2],      // quantity sold
//            revenue: p[3],  // revenue amount
//        }));
//    }
//
//    Quote() {
...
//    onClear() {
//        document.getElementById('period').value = "";
//        this._fetch_data("", this.state.selected, this.category.selected);
//    }
//
//    onChangeCountry(ev) {
//        this.state.selected = ev.target.value;
...
//    onClickTeam(ev) {
//        const btn = String(ev.target.value);
//        this.sales_team = btn === 'product' ? this.result.sales_team_prod : this.result.sales_team;
//        this.salesBarGraph();
//    }
//
//    onClickCus(ev) {
//        const btn = String(ev.target.value);
//        this.top_customers = btn === 'product' ? this.result.top_customers_prod : this.result.top_customers;
//        this.topBarGraph();
//    }
//
//    onClickBtn(ev) {
//        const btn = String(ev.target.value);
//        this.order_state = btn === 'invoice' ? this.result.invoice_state : this.result.order_state;
//        this.OSGraph();
//    }
//
//    onSelectChange(ev) {
//        const op = ev.target.value;
//        this.ls_product = op === "highest" ? this.result.hs_product : this.result.ls_product;
//        this.LSPLineGraph();
//    }
//
...
//                data: {
//                    labels: this.sales_person.map(c => c[1]),
//                    datasets: [{ data: this.sales_person.map(c => c[2]), backgroundColor: ['#00C2A8', '#5C5CFF'] }]
//                },
//                options: { plugins: { legend: { labels: { color: "white" } } }, responsive: true }
//            });
//        } else {
//            this.ppChart.data.labels = this.sales_person.map(c => c[1]);
//            this.ppChart.data.datasets[0].data = this.sales_person.map(c => c[2]);
...
//                data: {
//                    labels: this.top_customers.map(c => c[1]),
//                    datasets: [{ label: 'Total Sales', data: this.top_customers.map(c => c[2]), backgroundColor: '#E6BE8A', borderColor: 'black', borderWidth: 2 }]
//                },
//                options: {
...
//            this.lsChart = new Chart(ctx, {
//                type: 'line',
//                data: { labels: this.ls_product.map(c => c[0]), datasets: [{ data: this.ls_product.map(c => c[1]), label: "Products sold", borderColor: '#3D8BFF', backgroundColor: '#3D8BFF', pointBackgroundColor: '#1c1c1c' }] },
//                options: {
//                    responsive: true, indexAxis: 'y', animation: { duration: 1000, easing: 'easeOutCubic' },
//                    scales: {
...
//            this.osChart = new Chart(ctx, {
//                type: 'line',
//                data: { labels: this.order_state.map(c => c[0]), datasets: [{ data: this.order_state.map(c => c[1]), label: "Total Amount", borderColor: '#009081', backgroundColor: '#009081', pointBackgroundColor: '#009081' }] },
//                options: {
//                    responsive: true, indexAxis: 'y', animation: { duration: 1000, easing: 'easeOutCubic' },
//                    scales: {
//                        x: { beginAtZero: true, ticks: { color: '#FFFFFF' } },
